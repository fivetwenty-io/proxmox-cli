...
func newBackupIncludedVolumesCmd() *cobra.Command {
	return &cobra.Command{
		...
		Example: `  pmx pve cluster backup included-volumes backup-abc123`,
...
func (bf *backupFlags) register(cmd *cobra.Command) {
	...
	fl.BoolVar(&bf.protected, "protected", false,
		"mark the resulting backups as protected; protected backups are excluded from prune and "+
	...
	fl.Int64Var(&bf.ionice, "ionice", 7, "ionice priority for the backup process (0-8, BFQ scheduler only)")
	...
	fl.Int64Var(&bf.zstd, "zstd", 1, "number of zstd threads (0 uses half the available cores)")
...
func newBackupCreateCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  pmx pve cluster backup create --vmid 100,101 --schedule "sat 03:00" --storage local-lvm
  pmx pve cluster backup create --all --schedule "02:30" --storage local-lvm
  pmx pve cluster backup create --all --schedule "02:30" --storage local-lvm --notes-template "{{guestname}}"
  pmx pve cluster backup create --vmid 100 --schedule "1st sun 04:00" --storage local-lvm --protected  # long-term copies, kept out of prune/retention`,
		Args: cobra.NoArgs,
		RunE: func(cmd *cobra.Command, _ []string) error {
			deps := cli.GetDeps(cmd)
