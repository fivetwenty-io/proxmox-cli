...
def run(ctx: Ctx) -> None:
    ...
    users = ctx.check("user list", "pve", "access", "user", "list", validate=is_list)
    roles = ctx.check("role list", "pve", "access", "role", "list", validate=is_list)
    groups = ctx.check("group list", "pve", "access", "group", "list", validate=is_list)
    ctx.check("acl list", "pve", "access", "acl", "list", validate=is_list)
    ...
    ctx.check("permissions (self)", "pve", "access", "permissions", validate=is_perm_tree)

    # Authentication realms (domains). `pam` and `pve` always exist, so a get of
    # the first listed realm is unconditional.
    domains = ctx.check("domain list", "pve", "access", "domain", "list", validate=is_list)
    realm = None
    if domains.rc == 0:
        try:
            ...
            realm = None
    if realm:
        ctx.check("domain get", "pve", "access", "domain", "get", str(realm))
    else:
    ...
    # are conditional (◑).
    tfa = ctx.check("tfa list", "pve", "access", "tfa", "list", validate=is_list)
    tfa_user = None
    if tfa.rc == 0:
        try:
            ...
            tfa_user = None
    if tfa_user:
        usr_entries = ctx.check("tfa get", "pve", "access", "tfa", "get", str(tfa_user))
        ...
        if usr_entries.rc == 0:
            try:
                ...
                tfa_entry_id = None
        if tfa_entry_id:
            ctx.check("tfa get-entry", "pve", "access", "tfa", "get-entry",
    ...
    ctx.check("tfa types", "pve", "access", "tfa", "types", "root@pam",
    ...
    ctx.check("openid list", "pve", "access", "openid", "list", validate=is_list)
    ...
    if users.rc == 0:
        try:
            ...
            uid = None
    if uid:
        ctx.check("user get", "pve", "access", "user", "get", str(uid))
        tokens = ctx.check("user token list", "pve", "access", "user", "token", "list", str(uid))
        ...
        if tokens.rc == 0:
            try:
                ...
                tid = None
        if tid:
            ctx.check("user token get", "pve", "access", "user", "token", "get", str(uid), str(tid))
        else:
    ...
    if roles.rc == 0:
        try:
            ...
            rid = None
    if rid:
        ctx.check("role get", "pve", "access", "role", "get", str(rid))
    else:
    ...
    if groups.rc == 0:
        try:
            ...
            gid = None
    if gid:
        ctx.check("group get", "pve", "access", "group", "get", str(gid))
    else:
    ...
    ctx.defer("tfa delete",
              ...
              "pmx pve access tfa delete <user> <id> --yes",
              isolation=True, live_covered=True)
