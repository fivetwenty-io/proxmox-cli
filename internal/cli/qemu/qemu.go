...
// actually runs on, consulting the cluster inventory unless --node was passed
// explicitly. An ambient default node (PMX_NODE or the context default-node)
// is not trusted as the VM's location. See cli.ResolveGuest for the full
// lookup semantics.
...
func resolveGuestSource(cmd *cobra.Command, deps *cli.Deps, target string) (vmid, node string, err error) {
	vmid, node, err = cli.ResolveGuest(cmd.Context(), deps, target, cli.GuestQemu)
