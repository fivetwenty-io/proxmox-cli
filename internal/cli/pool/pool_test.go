...
// TestPoolShow verifies pool show uses GET /pools/{poolid} (deprecated
// single-object endpoint), distinct from pool get's GET /pools?poolid=<id>.
func TestPoolShow(t *testing.T) {
	f := testhelper.NewFakePVE(t)
	var rec []recordedRequest
...
// TestPoolShowTypeFilter verifies the --type flag is forwarded as a query
// parameter to GET /pools/{poolid}.
func TestPoolShowTypeFilter(t *testing.T) {
	f := testhelper.NewFakePVE(t)
	var rec []recordedRequest
	mustNewClientAndRecord(f, &rec, "GET /api2/json/pools/prod", map[string]any{
		"comment": "production",
	...
	out, err := run(t, f, "", "show", "prod", "--type", "qemu")
	require.NoError(t, err)
	require.Contains(t, out, "prod")
	require.Len(t, rec, 1)
...
// TestPoolShowNotFound covers the error path when the pool does not exist:
// Proxmox VE returns a non-2xx status with an error envelope, which the
// client surfaces as an error before any decoding of pool fields happens.
func TestPoolShowNotFound(t *testing.T) {
	f := testhelper.NewFakePVE(t)
	var rec []recordedRequest
	mustNewClientAndRecord(f, &rec, "GET /api2/json/pools/ghost", nil, 500)

	_, err := run(t, f, "", "show", "ghost")
	require.Error(t, err)
	require.ErrorContains(t, err, "show pool")
	require.Len(t, rec, 1)
	require.Equal(t, http.MethodGet, rec[0].method)
	require.Equal(t, "/api2/json/pools/ghost", rec[0].path)
}

func TestPoolShowError(t *testing.T) {
	f := testhelper.NewFakePVE(t)
	var rec []recordedRequest
	mustNewClientAndRecord(f, &rec, "GET /api2/json/pools/prod", nil, 404)

	_, err := run(t, f, "", "show", "prod")
	require.Error(t, err)
	require.Len(t, rec, 1)
}
