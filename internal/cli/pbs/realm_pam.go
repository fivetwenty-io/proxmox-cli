...
// rather than `ls`/`add`/`delete` — the PAM realm cannot be created or
// deleted, only its comment/default flag configured.
...
func newRealmPamShowCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			resp, err := deps.PBS.Config.ListAccessPam(cmd.Context())
			if err != nil {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if !anyFlagChanged(cmd.Flags()) {
				return fmt.Errorf("update PAM realm: no changes given: pass at least one flag")
			}
			...
			params := &pbsconfig.UpdateAccessPamParams{}
			...
			_, err := deps.PBS.Config.UpdateAccessPam(cmd.Context(), params)
			if err != nil {
