...
.PHONY: test-e2e
test-e2e: ## Run end-to-end happy-path sweep of all command trees (CONTEXT=lab-pmx; PBS_CONTEXT=<pbs ctx> opts into the pbs tree; PDM_CONTEXT=<pdm ctx> opts into the pdm tree)
...
test-e2e-mutate: ## Run the e2e sweep plus the destructive qemu/lxc verb matrix (CONTEXT=lab-pmx; PBS_CONTEXT=<pbs ctx> opts into the pbs tree; PDM_CONTEXT=<pdm ctx> opts into the pdm tree)
