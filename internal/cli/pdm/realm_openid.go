...
func newRealmOpenidLsCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Example: "  pmx pdm realm openid ls",
		Args:    cobra.NoArgs,
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			table, err := cli.DecodePairedRows[realmOpenidListEntry](items, "OpenID realm")
			if err != nil {
				return err
			}
			for i := range table {
				stripRealmOpenidSecrets(table[i].Raw)
			}
...
func newRealmOpenidAddCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "Create a new OpenID Connect authentication realm configuration " +
			...
			"explicitly set.",
		Example: `  pmx pdm realm openid add company --issuer-url https://idp.example.com \
    --client-id pdm-cli --client-key '${OPENID_CLIENT_SECRET}'`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			deps := cli.GetDeps(cmd)
			realm := args[0]
...
func newRealmOpenidUpdateCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  pmx pdm realm openid update company --scopes 'openid email profile'",
		Args:    cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			deps := cli.GetDeps(cmd)
			realm := args[0]
...
func newRealmOpenidDeleteCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  pmx pdm realm openid delete company --yes",
		Args:    cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			deps := cli.GetDeps(cmd)
			realm := args[0]
