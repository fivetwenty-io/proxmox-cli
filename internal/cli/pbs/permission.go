...
func newPermissionLsCmd() *cobra.Command {
  ...
  pmx pbs permission ls --auth-id alice@pbs --path /datastore/tank`,
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			params := &pbsaccess.ListPermissionsParams{}
			...
			resp, err := deps.PBS.Access.ListPermissions(cmd.Context(), params)
			if err != nil {
			...
			for _, p := range paths {
				...
				for _, priv := range privs {
					rows = append(rows, []string{p, priv, strconv.FormatBool(tree[p][priv])})
				}
			}
