...
// newSecurityTpmCmd builds `pve qemu security tpm` and its show/add/remove
// sub-commands.
...
// newSecurityTpmShowCmd builds `pve qemu security tpm show <vmid|name>`.
...
// newSecurityTpmAddCmd builds `pve qemu security tpm add <vmid|name>`.
func newSecurityTpmAddCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "Allocate a TPM state disk (tpmstate0). The version defaults to v2.0 — the current, " +
			...
			"Example: pve qemu security tpm add win11 --storage local-lvm",
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			deps := cli.GetDeps(cmd)
			fl := cmd.Flags()
			...
			if tp.Present {
				...
				return fmt.Errorf(
					"VM %s already has a TPM state device (version %s); the version cannot be changed "+
						"in place — remove it first with 'pve qemu security tpm remove %s --force' "+
...
// newSecurityTpmRemoveCmd builds `pve qemu security tpm remove <vmid|name>`.
