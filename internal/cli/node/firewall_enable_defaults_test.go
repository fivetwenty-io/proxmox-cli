...
func TestFirewallRuleEnableWireDefaults(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			method, path := http.MethodPost, "/api2/json/nodes/pve1/firewall/rules"
			args := []string{"--node", "pve1", "node", "firewall", "rules", "create", "--type", "in", "--action", "ACCEPT"}
			if tc.update {
				method = http.MethodPut
				path += "/0"
				args = []string{"--node", "pve1", "node", "firewall", "rules", "update", "0", "--comment", "keep activation unchanged"}
			}
			...
			root, _, prefix := newNodeRoot(t, f, output.FormatJSON, exec.Fake())
			root.SetArgs(append(prefix, args...))
			err := root.Execute()
			if tc.reject {
