...
| `lab ceph init` | — | — | deferred — runs `pveceph init` on a lab's node 0 through the nested cluster's own API context; needs the dedicated lab-pmx destructive test lab as the standing target |
