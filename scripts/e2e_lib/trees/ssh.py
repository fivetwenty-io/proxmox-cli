"""ssh: top-level SSH passthrough wrapper (`pmx ssh <node> [ssh-option...] [command...]`).

Delegates to the same `remote.RunSSH` code path as `pmx node ssh` (see
`node.py`), so every invocation execs the real `ssh` binary and opens a live
session on the resolved node. `pmx node ssh` already exercises that shared
...
def run(ctx: Ctx) -> None:
    ctx.defer(
        ...
        "head-less by the read-only sweep; shares the `pmx node ssh` code path "
