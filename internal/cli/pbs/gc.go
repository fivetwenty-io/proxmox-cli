	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			entry := gcStatusEntry{
				...
				DiskChunks:     resp.DiskChunks,
				Duration:       resp.Duration,
				IndexDataBytes: resp.IndexDataBytes,
				IndexFileCount: resp.IndexFileCount,
				LastRunEndtime: resp.LastRunEndtime,
				LastRunState:   resp.LastRunState,
				NextRun:        resp.NextRun,
				PendingBytes:   resp.PendingBytes,
				PendingChunks:  resp.PendingChunks,
				RemovedBad:     resp.RemovedBad,
				RemovedBytes:   resp.RemovedBytes,
				RemovedChunks:  resp.RemovedChunks,
				Schedule:       resp.Schedule,
				StillBad:       resp.StillBad,
				Store:          resp.Store,
				Upid:           resp.Upid,
			}
			if entry.Store == "" {
