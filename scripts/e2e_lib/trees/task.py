...
def run(ctx: Ctx) -> None:
    ...
    lst = ctx.check("list", "pve", "task", "list", node=n, validate=is_list)
    ...
    ctx.check("cluster-list", "pve", "task", "cluster-list", validate=is_list)
    ...
    if upid is None:
        ...
        ctx.check("log", "pve", "task", "log", str(upid), node=n)
        # status: the node is parsed from the UPID itself, so no --node is needed.
        ctx.check("status", "pve", "task", "status", str(upid))

    ctx.defer("wait", "blocks until a task finishes", "pmx pve task wait <upid> --node <node>")
    ...
    ctx.defer("stop", "aborts a running task — covered live by `e2e --mutate`",
              "pmx pve task stop <upid> --node <node>", live_covered=True)
