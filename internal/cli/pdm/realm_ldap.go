...
// /config/access/ldap: the LDAP realm's full configuration. The "password"
// bind credential is deliberately absent from this struct: config_gen.go's
// GetAccessLdapResponse (v3.6.0) has no Password field, and the API
// schema's GET /config/access/ldap/{realm} returns.properties list has no
// "password" key either — the server never returns it, so there is nothing
// to strip.
type realmLdapListEntry struct {
	...
	Port                *pveclient.PVEInt  `json:"port,omitempty"`
	...
	Verify              *pveclient.PVEBool `json:"verify,omitempty"`
}

// newRealmLdapLsCmd builds `pmx pdm realm ldap ls` — list configured LDAP
// realms (GET /config/access/ldap).
func newRealmLdapLsCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:     "ls",
		Short:   "List LDAP realm configurations",
		Long:    "List the LDAP authentication realms configured on this Proxmox Datacenter Manager (GET /config/access/ldap).",
		Example: "  pmx pdm realm ldap ls",
		Args:    cobra.NoArgs,
		RunE: func(cmd *cobra.Command, _ []string) error {
			deps := cli.GetDeps(cmd)

			resp, err := deps.PDM.Config.ListAccessLdap(cmd.Context())
			if err != nil {
				return fmt.Errorf("list LDAP realms: %w", err)
			}

			items := rawItemsOf(resp)
			table, err := cli.DecodePairedRows[realmLdapListEntry](items, "LDAP realm")
			if err != nil {
				return err
			}
...
// newRealmLdapShowCmd builds `pmx pdm realm ldap show <realm>` — show a
...
func newRealmLdapShowCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  pmx pdm realm ldap show company",
		Args:    cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			deps := cli.GetDeps(cmd)
			realm := args[0]

			resp, err := deps.PDM.Config.GetAccessLdap(cmd.Context(), realm)
			if err != nil {
...
func (lf *realmLdapFlags) registerCommon(cmd *cobra.Command) {
	...
	f.StringVar(&lf.syncAttributes, "sync-attributes", "",
		"comma-separated key=value pairs mapping LDAP attributes to PDM user fields")
...
func (lf *realmLdapFlags) applyCreate(cmd *cobra.Command, p *pdmconfig.CreateAccessLdapParams) {
	fl := cmd.Flags()
...
func (lf *realmLdapFlags) applyUpdate(cmd *cobra.Command, p *pdmconfig.UpdateAccessLdapParams) {
	fl := cmd.Flags()
...
// newRealmLdapAddCmd builds `pmx pdm realm ldap add <realm>` — create an
// LDAP realm configuration (POST /config/access/ldap). --base-dn, --server1,
// and --user-attr are required; every other option is optional and only
...
func newRealmLdapAddCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "Create a new LDAP authentication realm configuration (POST " +
			...
			"explicitly set.",
		Example: `  pmx pdm realm ldap add company --server1 ldap1.example.com \
    --base-dn "dc=example,dc=com" --user-attr uid --bind-dn "cn=svc,dc=example,dc=com" --password '${LDAP_BIND_PASSWORD}'`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			deps := cli.GetDeps(cmd)
			realm := args[0]
			...
			params := &pdmconfig.CreateAccessLdapParams{
				Realm:    realm,
				BaseDn:   lf.baseDn,
				Server1:  lf.server1,
			...
			err := deps.PDM.Config.CreateAccessLdap(cmd.Context(), params)
			if err != nil {
...
// newRealmLdapUpdateCmd builds `pmx pdm realm ldap update <realm>` — update
...
func newRealmLdapUpdateCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  pmx pdm realm ldap update company --server2 ldap2.example.com",
		Args:    cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			deps := cli.GetDeps(cmd)
			realm := args[0]
			...
			if cmd.Flags().Changed("delete") {
				if slices.Contains(lf.del, "") {
					return fmt.Errorf("--delete: property name must not be empty")
				}
			}

			params := &pdmconfig.UpdateAccessLdapParams{}
			lf.applyUpdate(cmd, params)

			// UpdateAccessLdap returns only an error (config_gen.go:61, v3.6.0),
			// unlike the PBS analog's UpdateAccessLdap, which also returns the
			// updated realm object.
			err := deps.PDM.Config.UpdateAccessLdap(cmd.Context(), realm, params)
			if err != nil {
...
// newRealmLdapDeleteCmd builds `pmx pdm realm ldap delete <realm>` — remove
...
func newRealmLdapDeleteCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  pmx pdm realm ldap delete company --yes",
		Args:    cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			deps := cli.GetDeps(cmd)
			realm := args[0]
			...
			params := &pdmconfig.DeleteAccessLdapParams{}
			if cmd.Flags().Changed("digest") {
				params.Digest = new(digest)
			}

			err := deps.PDM.Config.DeleteAccessLdap(cmd.Context(), realm, params)
			if err != nil {
