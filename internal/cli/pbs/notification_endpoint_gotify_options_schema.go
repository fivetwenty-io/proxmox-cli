...
//go:generate go run github.com/fivetwenty-io/proxmox-cli/cmd/optionsgen -source pbs-apidoc.json -path /config/notifications/endpoints/gotify -verb POST -symbol notifGotifyOptionSchemas -exclude "name,token" -out notification_endpoint_gotify_options_schema_gen.go
