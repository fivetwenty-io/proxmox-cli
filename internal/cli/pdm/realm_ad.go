...
// newRealmAdCmd builds `pmx pdm realm ad` — manage Active Directory realm
...
// /config/access/ad: the AD realm's full configuration. The "password" bind
// credential is deliberately absent from this struct: config_gen.go's
// GetAccessAdResponse (v3.6.0) has no Password field, and the API schema's
// GET /config/access/ad/{realm} returns.properties list has no "password"
// key either — the server never returns it, so there is nothing to strip.
...
// newRealmAdLsCmd builds `pmx pdm realm ad ls` — list configured AD realms
// (GET /config/access/ad).
func newRealmAdLsCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:     "ls",
		Short:   "List Active Directory realm configurations",
		Long:    "List the Active Directory authentication realms configured on this Proxmox Datacenter Manager (GET /config/access/ad).",
		Example: "  pmx pdm realm ad ls",
		Args:    cobra.NoArgs,
		RunE: func(cmd *cobra.Command, _ []string) error {
			deps := cli.GetDeps(cmd)

			resp, err := deps.PDM.Config.ListAccessAd(cmd.Context())
			if err != nil {
...
// newRealmAdShowCmd builds `pmx pdm realm ad show <realm>` — show a single AD
...
func newRealmAdShowCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  pmx pdm realm ad show company",
		Args:    cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			deps := cli.GetDeps(cmd)
			realm := args[0]

			resp, err := deps.PDM.Config.GetAccessAd(cmd.Context(), realm)
			if err != nil {
...
func (af *realmAdFlags) applyCreate(cmd *cobra.Command, p *pdmconfig.CreateAccessAdParams) {
	fl := cmd.Flags()
...
func (af *realmAdFlags) applyUpdate(cmd *cobra.Command, p *pdmconfig.UpdateAccessAdParams) {
	fl := cmd.Flags()
	if fl.Changed("base-dn") {
		p.BaseDn = &af.baseDn
	}
...
// newRealmAdAddCmd builds `pmx pdm realm ad add <realm>` — create an AD
// realm configuration (POST /config/access/ad). --server1 is required; every
// other option is optional and only forwarded when explicitly set.
func newRealmAdAddCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  pmx pdm realm ad add company --server1 ad1.example.com \
    --base-dn "dc=example,dc=com" --bind-dn "cn=svc,dc=example,dc=com" --password '${AD_BIND_PASSWORD}'`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			deps := cli.GetDeps(cmd)
			realm := args[0]
			...
			params := &pdmconfig.CreateAccessAdParams{
				Realm:   realm,
				Server1: af.server1,
			}
			af.applyCreate(cmd, params)

			err := deps.PDM.Config.CreateAccessAd(cmd.Context(), params)
			if err != nil {
...
// newRealmAdUpdateCmd builds `pmx pdm realm ad update <realm>` — update an AD
...
func newRealmAdUpdateCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  pmx pdm realm ad update company --server2 ad2.example.com",
		Args:    cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			deps := cli.GetDeps(cmd)
			realm := args[0]
			...
			params := &pdmconfig.UpdateAccessAdParams{}
			af.applyUpdate(cmd, params)

			// UpdateAccessAd returns only an error (config_gen.go:46, v3.6.0),
			// unlike the PBS analog's UpdateAccessAd, which also returns the
			// updated realm object.
			err := deps.PDM.Config.UpdateAccessAd(cmd.Context(), realm, params)
			if err != nil {
...
// newRealmAdDeleteCmd builds `pmx pdm realm ad delete <realm>` — remove an AD
...
func newRealmAdDeleteCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  pmx pdm realm ad delete company --yes",
		Args:    cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			deps := cli.GetDeps(cmd)
			realm := args[0]
