...
//go:generate go run github.com/fivetwenty-io/proxmox-cli/cmd/optionsgen -source pbs-apidoc.json -path /config/notifications/endpoints/webhook -verb POST -symbol notifWebhookOptionSchemas -exclude "name,secret" -out notification_endpoint_webhook_options_schema_gen.go
