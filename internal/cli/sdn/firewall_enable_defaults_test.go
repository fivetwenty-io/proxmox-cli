...
func TestFirewallRuleEnableWireDefaults(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			method, path := http.MethodPost, "/api2/json/cluster/sdn/vnets/pmxcli0/firewall/rules"
			args := []string{"vnet", "firewall", "rules", "create", "pmxcli0", "--type", "in", "--action", "ACCEPT"}
			if tc.update {
				method = http.MethodPut
				path += "/0"
				args = []string{"vnet", "firewall", "rules", "set", "pmxcli0", "0", "--comment", "keep activation unchanged"}
			}
