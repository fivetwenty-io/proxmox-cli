...
// TestClusterStatus_Table verifies that `pmx pve cluster status` queries
...
// TestClusterResources_TypeFilter verifies that `pmx pve cluster resources --type vm`
...
// TestClusterNextID_Plain verifies `pmx pve cluster next-id` returns the next VMID.
