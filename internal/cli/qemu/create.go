...
func newCreateCmd() *cobra.Command {
  ...
  pmx pve qemu create 100 --cores 4 --memory 4096 --start`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			set("name", func() { params.Name = new(name) })
			set("pool", func() { params.Pool = new(pool) })
			set("tags", func() { params.Tags = new(tags) })
			set("start", func() { params.Start = new(start) })
			set("onboot", func() { params.Onboot = new(onboot) })
			set("startup", func() { params.Startup = new(startup) })
			set("lock", func() { params.Lock = new(lock) })

			set("cores", func() { params.Cores = new(cores) })
			set("sockets", func() { params.Sockets = new(sockets) })
			set("vcpus", func() { params.Vcpus = new(vcpus) })
			set("cpu", func() { params.Cpu = new(cpu) })
			set("cpulimit", func() { params.Cpulimit = &cpulimit })
			set("cpuunits", func() { params.Cpuunits = new(cpuunits) })
			set("affinity", func() { params.Affinity = new(affinity) })
			set("memory", func() { params.Memory = new(memory) })
			set("balloon", func() { params.Balloon = new(balloon) })
			set("shares", func() { params.Shares = new(shares) })
			set("numa", func() { params.Numa = new(numa) })
			set("hugepages", func() { params.Hugepages = new(hugepages) })
			set("keephugepages", func() { params.Keephugepages = new(keephugepages) })
			set("allow-ksm", func() { params.AllowKsm = new(allowKsm) })
			set("smbios1", func() { params.Smbios1 = new(smbios1) })
			set("machine", func() { params.Machine = new(machine) })
			set("arch", func() { params.Arch = new(arch) })
			set("ostype", func() { params.Ostype = new(ostype) })
			set("bios", func() { params.Bios = new(bios) })
			set("efidisk0", func() { params.Efidisk0 = new(efidisk0) })
			set("tpmstate0", func() { params.Tpmstate0 = new(tpmstate0) })
			set("vga", func() { params.Vga = new(vga) })
			set("scsihw", func() { params.Scsihw = new(scsihw) })
			set("boot", func() { params.Boot = new(boot) })
			set("acpi", func() { params.Acpi = new(acpi) })
			set("kvm", func() { params.Kvm = new(kvm) })
			set("freeze", func() { params.Freeze = new(freeze) })
			set("localtime", func() { params.Localtime = new(localtime) })
			set("tablet", func() { params.Tablet = new(tablet) })
			set("tdf", func() { params.Tdf = new(tdf) })
			set("reboot", func() { params.Reboot = new(reboot) })
			set("protection", func() { params.Protection = new(protection) })
			set("template", func() { params.Template = new(template) })
			set("unique", func() { params.Unique = new(unique) })
			set("force", func() { params.Force = new(force) })
			set("hotplug", func() { params.Hotplug = new(hotplug) })
			set("hookscript", func() { params.Hookscript = new(hookscript) })
			set("watchdog", func() { params.Watchdog = new(watchdog) })
			set("rng0", func() { params.Rng0 = new(rng0) })
			set("audio0", func() { params.Audio0 = new(audio0) })
			set("keyboard", func() { params.Keyboard = new(keyboard) })
			set("spice-enhancements", func() { params.SpiceEnhancements = new(spiceEnhance) })
			set("amd-sev", func() { params.AmdSev = new(amdSev) })
			set("intel-tdx", func() { params.IntelTdx = new(intelTdx) })
			set("ivshmem", func() { params.Ivshmem = new(ivshmem) })
			set("args", func() { params.Args = new(kvmArgs) })
			set("vmgenid", func() { params.Vmgenid = new(vmgenid) })
			set("startdate", func() { params.Startdate = new(startdate) })
			set("vmstatestorage", func() { params.Vmstatestorage = new(vmstatestorage) })

			set("storage", func() { params.Storage = new(storage) })
			set("description", func() { params.Description = new(description) })
			set("archive", func() { params.Archive = new(archive) })
			set("live-restore", func() { params.LiveRestore = new(liveRestore) })
			set("import-working-storage", func() { params.ImportWorkingStorage = new(importWorkingStorage) })
			set("bwlimit", func() { params.Bwlimit = new(bwlimit) })
			set("migrate-downtime", func() { params.MigrateDowntime = &migrateDowntime })
			set("migrate-speed", func() { params.MigrateSpeed = new(migrateSpeed) })
			set("ha-managed", func() { params.HaManaged = new(haManaged) })

			set("agent", func() { params.Agent = new(agent) })
			set("autostart", func() { params.Autostart = new(autostart) })
			set("cdrom", func() { params.Cdrom = new(cdrom) })
			set("ciuser", func() { params.Ciuser = new(ciuser) })
			set("cipassword", func() { params.Cipassword = new(cipassword) })
			set("citype", func() { params.Citype = new(citype) })
			set("ciupgrade", func() { params.Ciupgrade = new(ciupgrade) })
			set("cicustom", func() { params.Cicustom = new(cicustom) })
			set("nameserver", func() { params.Nameserver = new(nameserver) })
			set("searchdomain", func() { params.Searchdomain = new(searchdomain) })
			set("sshkeys", func() { params.Sshkeys = new(encodeSSHKeys(sshkeys)) })
