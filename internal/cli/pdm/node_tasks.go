...
type nodeTaskEntry struct {
	...
	// Id and Type carry the tags PDM actually sends. The API documents the
	// task schema with "id" and "type", but a live PDM 1.1 answers with
	// "worker_id" and "worker_type", so both columns were blank on every row
	// while the values sat unread in the response.
